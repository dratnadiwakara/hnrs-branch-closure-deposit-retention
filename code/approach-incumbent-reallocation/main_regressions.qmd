...
*Unit: bank-county-year. LHS: $(dep^{remain}_{t+1} - dep^{remain}_{t-1}) / dep^{total}_{t-1}$. Treatment: own closing deposits / total bank-county deposits. Excludes M&A-related closures and top-5% intensity. FE: bank×year + county×year. SE: clustered at bank.*

```{r table7}
#| output: true
reg_main <- setDT(load_latest(REG_MAIN_DATA, "^reg_main_.*\\.rds$"))
